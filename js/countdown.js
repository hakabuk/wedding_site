...
// Usage:
var test = new Countdown({ time: 1500000 });
...
if (!Array.prototype.indexOf)
	Array.prototype.indexOf = function(c) {
		...
		arguments.length > 1 &&
			...
			i != i
				? (i = 0)
				: i != 0 && i != Infinity && i != -Infinity && (i = (i > 0 || -1) * Math.floor(Math.abs(i))));
		if (i >= a) return -1;
...
(function() {
	...
	c.isNull = function(a) {
		return a === '' || a === null || a === void 0 || typeof a == 'undefined' || a == 'undefined' || a == 'null'
			? true
			: false;
	};
	c.clone = function(a) {
	...
	c.getXYWH = function(a) {
		...
		if (a) {
			for (
				var f = a.offsetWidth, c = a.offsetHeight, g = jbeeb.Browser.touch;
				a && !isNaN(a.offsetLeft) && !isNaN(a.offsetTop);

			)
				g
					? ((b += (a.offsetLeft || 0) - (a.scrollLeft || 0)), (d += (a.offsetTop || 0) - (a.scrollTop || 0)))
					: ((b += a.offsetLeft || 0), (d += a.offsetTop || 0)),
					(a = a.offsetParent);
			g &&
				((a = window.scrollY != null ? window.scrollY : window.pageYOffset),
				(b += window.scrollX != null ? window.scrollX : window.pageXOffset),
				(d += a));
		}
	...
	c.contains = function(a, b) {
		var d = {},
			f = { x: a.x, y: a.y, w: a.width, h: a.height },
	...
	c.getAttributes = function(a) {
		...
		if ((a = a.attributes)) {
			for (var d = a.length, c = 0; c < d; c++)
				jbeeb.Browser.ie
					? a[c].specified && (b[a[c].nodeName] = a[c].nodeValue.toString())
					: (b[a[c].nodeName] = a[c].value ? a[c].value.toString() : a[c].nodeValue.toString());
			return b;
		} else return {};
	};
	jbeeb.Utils = c;
		...
		if (a && b && (b = b[a])) {
			...
			for (var d = 0; d < b.length; d++) {
				...
				if (c.fn) {
					...
					h.length
						? c.scope ? c.fn.apply(c.scope, h) : c.fn.apply(null, h)
						: c.scope ? c.fn.call(c.scope) : c.fn();
				}
			}
		}
	};
	...
	if (!jbeeb.ready)
		jbeeb.ready = (function() {
			var b,
				...
				c = d.documentElement,
				h = c.doScroll,
				g = (h ? /^loaded|^c/ : /^loaded|c/).test(d.readyState);
			a = function(b) {
				try {
					(b = d.getElementsByTagName('body')[0].appendChild(d.createElement('span'))),
...
(function() {
	...
	e.start = function() {
		if (!this.state)
			(this.state = 1),
				(this.bo = []),
				this.bo.push((this.aR = c())),
				this.ag ? this.bK(this.cW) : this.bK(this.af),
				this.bw();
	};
	e.getFPS = function() {
	...
	e.bw = function() {
		if (this.aF == null)
			this.ag
				? (b(this.bN), (this.aF = true))
				: (this.aF && clearTimeout(this.aF), (this.aF = setTimeout(this.bN, this.L)));
	};
	e.bK = function(a) {
		this.bN = a.bind(this);
	};
		...
		function c(a, b) {
				...
				k.substr(0, 1) == '/'
					...
					: k.substr(0, 3) == '../'
						? ((l = l.parenturl.split('/')),
						  (k = k.split('../')),
						  (o = k.pop()),
						  l.splice(l.length - k.length, k.length, o),
						  (k = l.join('/')))
						: (k = l.pathurl + (e ? '' : '/') + k));
			...
			z
				...
				: ((l += n + (n != '' ? '/' : '')),
				  ...
				  !t && !r && i.substr(-1) != '/' && (i += '/'));
			e === false &&
				(m.substr(-1) == '/' && (m = m.substr(0, m.length - 1)),
				u.substr(-1) == '/' && (u = u.substr(0, u.length - 1)),
				z ||
					(l.substr(-1) == '/' && (l = l.substr(0, l.length - 1)),
					q.substr(-1) == '/' && (q = q.substr(0, q.length - 1)),
...
(function() {
	...
	b.init = function(a) {
		...
		if (this.K)
			(b = this.K),
				...
				(b.WebkitTextSizeAdjust = '100%'),
				(b.msTextSizeAdjust = '100%'),
				(b.boxSizing = 'padding-box'),
				(b.backgroundClip = 'padding-box');
		a.editable || this.setSelectable(false);
		this.setCursor('inherit');
		if (a)
			(this.autoCenter = a.center),
				typeof a.flex != 'undefined' && this.setFlex(a.flex),
				typeof a.pin != 'undefined' && this.setPin(a.pin),
	...
	b.applySkin = function(a, b) {
		...
		e &&
			(typeof e == 'string'
				...
				: e.color != null &&
				  ((c = e.color || '#000000'),
				  (h = jbeeb.Utils.isNumber(e.alpha) ? e.alpha : 1),
				  (d = e.weight || 1),
				  (g = e.style || 'solid')));
	...
	b.bM = function() {
		...
		if (a) {
			...
			if (g) {
				...
				for (
					var j = j.alpha || 'v', k = jbeeb.Browser, o = [], p = [], v = g.length, z = k.oldWebkit, n = 0;
					n < v;
					n += 3
				) {
					var l = jbeeb.Utils.makeColor(g[n], g[n + 1]),
						q = g[n + 2];
					q > 100 ? (q = 100) : q < 0 && (q = 0);
				...
				if (k.modern)
					(b = k.cssPrefix),
						b == ''
							? ((b = 'linear-'), (j = (j == 'v' ? 'to bottom, ' : 'to right, ') + o.join(',')))
							: b == 'webkit' && z
								? ((g = p.join(',')),
								  (b = '-webkit-'),
								  (j =
										j == 'v'
											? 'linear, left top, left bottom, ' + g
											: 'linear, left top, right top, ' + g))
								: ((b = '-' + b + '-linear-'), (j = (j == 'v' ? 'top, ' : 'left, ') + o.join(','))),
						(b = b + 'gradient(' + j + ')');
					...
					b = '';
					for (n = 0; n < v; n += 3)
						jbeeb.Utils.makeColor(g[n], g[n + 1]),
							(b +=
								'<stop offset="' +
								g[n + 2] +
								'%" stop-color="' +
					...
					p +=
						'  <linearGradient id="' +
						j +
						'" gradientUnits="userSpaceOnUse" x1="0%" y1="0%" x2="' +
						g +
						'%" y2="' +
						o +
						'%">';
			...
			if (this.K)
				(g = this.K),
					(g.backgroundColor = e || ''),
					(g.backgroundImage = b || 'none'),
					(g.backgroundSize = d || ''),
					(g.backgroundRepeat = c || ''),
					(g.backgroundPosition = h || '');
		}
	};
	...
	b.setStroke = function(a, b, e, d) {
		...
		var h = this.style;
		a
			? ((e = a + 'px'), (b = jbeeb.Utils.makeColor(b, c)), (c = -a + 'px'), (a = -a + 'px'))
			: (a = c = b = e = d = '');
		h.borderStyle = d;
		h.borderWidth = e;
		h.borderColor = b;
	...
	b.center = function(a) {
		if ((this.parent || this.amStage) && this.width && this.height) {
			var b = this.x,
				...
				f;
			this.amStage
				? ((c = jbeeb.Utils.getXYWH(this.element.parentNode)), (f = c.w * 0.5), (c = c.h * 0.5))
				: ((c = this.parent), c.width || c.measure(), (f = c.width * 0.5), (c = c.height * 0.5));
			var h = this.width * 0.5,
				g = this.height * 0.5;
	...
	b.setOverflow = function(a) {
		...
		if (a == 'x' || a == 'y' || !a)
			a == 'x'
				? ((b = 'auto'), (e = 'hidden'))
				: a == 'y' && ((b = 'hidden'), (e = 'auto'), jbeeb.Browser.ie && this.setWidth(this.width + 20)),
				(this.style.overflowX = b),
				(this.style.overflowY = e);
		...
		if (!(b == [] && c == [] && d == [] && f == [])) {
			for (var b = c.concat(d, f, b), c = b.length, d = [], f = [], g = 0, j = 0; j < c; j++)
				g == 0
					...
					: g < 4
						? f.push(b[j] + 'px')
						: (f.push(jbeeb.Utils.makeColor(b[j], b[j + 1])), d.push(f.join(' ')), (f = []), ++j, (g = -1)),
					g++;
			d.length > 0 && (h = d.join(','));
		}
	...
	b.setBevel = function(a) {
		if (a)
			jbeeb.Utils.isNumber(a)
				? (a = { x: -a, y: -a, s1: 0, s2: 0, c1: '#FFFFFF', c2: '#000000', a1: 1, a2: 1 })
	...
	b.setFloat = function(a) {
		...
		if (this.K)
			(this.K.position = 'relative'),
				(this.K.left = null),
				(this.K.top = null),
				(this.K.cssFloat = a),
				(this.K.display = 'inline-block');
	};
	b.destroy = function() {
...
(function() {
	...
	b.applySkin = function(a, b) {
		...
		if (a.textColor) {
			var c = a.textColor,
				d = {};
			if (typeof c == 'string') d = { color: c, alpha: 1 };
			else if (((d = c), !d.color)) (d.color = null), (d.alpha = null);
	...
	b.setEditable = function(a) {
		a === 1
			? (this.amSM || this.setCursor('text'),
			  this.J ? this.J.removeAllEventListeners() : (this.J = new jbeeb.Keyboard(this.element)),
			  this.J.addEventListener('keydown', this.keyHandler, this),
			  this.J.addEventListener('keyup', this.keyHandler, this),
			  this.setOverflow('hidden'),
			  jbeeb.Utils.bindEvent(this.element, 'focus', this.setFocus.bind(this)),
			  jbeeb.Utils.bindEvent(this.element, 'blur', this.bE.bind(this)),
			  this.addMEL('mouseUp', this.setFocus, this))
			: (this.amSM || this.setCursor('default'),
			  this.J && this.J.removeAllEventListeners(),
	...
	b.keyHandler = function(a, b, c) {
		...
		d
			? ((this.text = this.aN && !this.multiline ? this.element.value : this.by.text),
			  c == 'keyup' && this.dispatchEvent('change', this, this.text))
			: this.J.block(a);
	};
	b.bE = function() {
	...
	b.setPadding = function(a) {
		...
		var c = '',
			...
			h = '';
		this.multiline
			? ((c = a + 'px'), (d = a + 'px'), (f = a + 'px'), (h = a + 'px'))
			: this.align == 'left' ? a && (c = a + 'px') : this.align == 'right' && a && (d = a + 'px');
		b.paddingLeft = c;
		b.paddingRight = d;
		b.paddingTop = f;
		b.paddingBottom = h;
	...
	b.da = function() {
		var a = this.font,
			b = this.textColor || {},
			b = jbeeb.Utils.makeColor(b.color, b.alpha),
	...
	b.setText = function(a) {
		if (this.element) {
			...
			if (this.aN && !this.multiline) this.element.value = a;
			else {
				if (!this.by) {
		...
		if (!(a == [] && b == [] && c == [] && d == [])) {
			for (var a = b.concat(c, a, d), b = a.length, c = [], d = [], h = 0, g = 0; g < b; g++)
				h == 0
					...
					: h < 4
						? d.push(a[g] + 'px')
						: (d.push(jbeeb.Utils.makeColor(a[g], a[g + 1])), c.push(d.join(' ')), (d = []), ++g, (h = -1)),
					h++;
			c.length > 0 && (f = c.join(','));
		}
		a = this.style;
	...
	b.ab = function(a) {
		if (a) {
			...
			if (a.stage) (this.amStage = 0), this.am(a);
				...
				this.style.position = 'relative';
				this.style.display =
					a.inline === true || a.inline == 'true' || a.inline === 1 ? 'inline-block' : 'block';
	...
	b.bq = function(a) {
		...
		if (a instanceof Date) a = new Date(a.getTime());
			...
			var b = a.year ? parseInt(a.year) : b.getFullYear(),
				...
				h = a.minute ? parseInt(a.minute) : 0,
...
(function() {
	var c = function(a) {
			...
			if (a.bkgd) {
				...
				h.stroke &&
					h.strokeColor &&
					(c = { weight: h.stroke || 1, color: h.strokeColor, alpha: h.strokeAlpha });
	...
	b.init = function() {
		...
		for (
			var c = 'second,minute,hour,day,month,year'.split(','),
				d = a.rangeLo ? a.rangeLo : 'second',
				f = a.rangeHi ? a.rangeHi : 'year',
				d = d.substr(-1) == 's' ? d.substr(0, d.length - 1) : d,
				f = f.substr(-1) == 's' ? f.substr(0, f.length - 1) : f,
				h = d,
				g = f,
				j = 0;
			j < c.length;
		...
		for (j = s = 0; j < c.length; j++) {
			...
			if (this.bU[y].use) {
				...
				if (this.T == 'flip') {
					var u =
							this.imagePhysicalWidth *
							((l - t * 2 - n * 2) / this.totalFlipDigits / this.imagePhysicalWidth),
						w = this.imagePhysicalHeight * (q / this.imagePhysicalHeight);
					m.time = new jbeeb.Container({ x: 0, y: 0, w: u * this.totalFlipDigits, h: w });
					for (var B = [], A = 0; A < this.totalFlipDigits; A++) {
						for (
							var C = new jbeeb.Container({ x: u * A + t * A, y: 0, w: u, h: w }), G = [], D = 0;
							D < 10;
							D++
						) {
					...
					(m.line = new jbeeb.Box({ x: 0, y: 0, w: v, h: o * 0.03, fill: '#000000' })),
						m.addChild(m.line),
						m.line.center();
				this.aY.addChild(m);
				if (!this.bv)
					(m.labels = new jbeeb.TextBox({
						x: s,
						y: o - f * 0.7,
						w: d,
						h: f * 0.7,
		...
		m = this.aV;
		this.T == 'flip'
			? (m.year && this.V('year', '00'),
			  m.month && this.V('month', '00'),
			  m.day && this.V('day', '00'),
			  m.hour && this.V('hour', '00'),
			  m.minute && this.V('minute', '00'),
			  m.second && this.V('second', '00'))
			: (m.year && m.year.time.setText('00'),
			  m.month && m.month.time.setText('00'),
			  m.day && m.day.time.setText('00'),
			  m.hour && m.hour.time.setText('00'),
			  m.minute && m.minute.time.setText('00'),
			  m.second && m.second.time.setText('00'),
			  this.ar());
		...
		if (a)
			for (b = a = 0; b < this.bV.length; b++)
				(c = this.bV[b]),
					...
					this.bb(f),
					c.setX(a),
					c.time.setWidth(c.width),
					c.time.center(),
					c.labels && (c.labels.setX(a), c.labels.setWidth(c.width)),
					c.line && (c.line.setWidth(c.width), c.line.center()),
					(a += c.width + this.aI);
	};
	b.bQ = function() {
		...
		var a = this.aV,
			b = this.aS;
		this.T == 'flip'
			? (a.year && this.V('year', b.year),
			  a.month && this.V('month', b.month),
			  a.day && this.V('day', b.day),
			  a.hour && this.V('hour', b.hour),
			  a.minute && this.V('minute', b.minute),
			  a.second && this.V('second', b.second))
			: (a.year && a.year.time.setText(b.year),
			  a.month && a.month.time.setText(b.month),
			  a.day && a.day.time.setText(b.day),
			  a.hour && a.hour.time.setText(b.hour),
			  a.minute && a.minute.time.setText(b.minute),
			  a.second && a.second.time.setText(b.second),
			  this.ar());
	};
	b.V = function(a, b) {
		for (var c = 0; c < this.totalFlipDigits; c++) {
			var d = this.aV[a].time.slot[c],
				f = this.bU[a],
				h = String(b).substr(c, 1),
				g = d.num[h];
			if (g) {
				if (f.prev[c] != h) {
				...
				if (f.ani[c]) {
					...
					this.aD
						? g.img[2].show()
						: (g.img[f.aniCount[c]].show(), f.aniCount[c]++, f.aniCount[c] > 2 && (f.ani[c] = false));
				}
				f.prev[c] = h;
			}
		}
